...
public class JMapServlet extends HttpServlet {
     ...
     * Or, you may want to override this function. This method get the
     * /WEB-INF/jmap-config.properties through the
    ...
    /**
     *
     * create and initialize the bean by the default rule. the rule to find the
     * bean is: packagePrefix + BEAN_PACKAGE + beanSimpleName create this
     * instance and call the abstract method init() if the beanSimpleName
     * contains '.', this name will be treated as full class name, so the
     * beanSimpleName will not add the package prefix name.
     *
     * Mar 8th 2014: replace the Class.forName to getClassLoader.loadClass This
     * is because the OSGi recommendation for classloader. refer to:
    ...
    /**
     * return the class name in String[0], action method name in String[1], hint
     * in the String[2]
     *
     * @param request
     * @return
     ...
     * controller's class name. If this application use the `/' as the servlet
     * path, the Class Root will be mapped. The config file
     * /WEB-INF/jmap-config.properties will define a key:
     * jmap.controller.package, this key will keep the package prefix name for
     * the controller Class.
     *
     * @param servletPath
     * @return the full name with package name and you can use
     * ClassLoader.loadClass to find the Class.
     */
